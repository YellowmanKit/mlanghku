...
public class Common_SlideShow : Page {

//	private string					_dynSelectedProjectId = "";
	private int						_dynCurrentIndex = 0;
...
//	private JSONNode				cardsNode = null;
	...
	private bool isDetectingAudioStop  = false;
	private bool isWaitingForAudio     = false; // v1.3
	private float waitingForAudioTimer = 0; // v1.3
...
//				}
				...
				//------------------------------------------------------------------
				// TODO: 2017/01/06
				// I should detected whether the audio file has downloaded or not.
				// If not, maybe wait for certain second and re-try.
				//
				// TODO: why _langRows[0]?  Can we choose?
				//------------------------------------------------------------------
				//_dynCurrentCardController._langRows[0].EventButtonPlaySound ();
				//isDetectingAudioStop = true;

				isWaitingForAudio = true;
				waitingForAudioTimer = 3; // wait for 3 seconds
				//------------------------------------------------------------------
			} else {
				NavigationBar.Instance.SetRightButton();
			}
...
//				state = PageState.END;
		...
		if (isDetectingAudioStop && !_audioSource.isPlaying) {
			isDetectingAudioStop = false;
			EventButtonNextCard ();
		}

		if (isWaitingForAudio) {
			if (_dynCurrentCardController._langRows[0]._btnPlay.interactable) {
				isWaitingForAudio = false;
				waitingForAudioTimer = 0;

				if (_dynCurrentCardController._langRows [0].TryToPlaySound () == true) {
					isDetectingAudioStop = true;
				} else {
					// show error and skip to next page
					...
					fail.buttonCompletedEvent_0 += PostSoundError;
					GameManager.Instance.AlertTasks.Add (fail);
				}
			} else {
				waitingForAudioTimer -= Time.deltaTime;
				if (waitingForAudioTimer <= 0) {
					waitingForAudioTimer = 0;
					// show error and skip to next page
					AlertTask fail = new AlertTask(
						GameManager.Instance.Language.getString("error", CaseHandle.FirstCharUpper)
	...
	public override	void Init (Dictionary<string, object> data) {
		Debug.Log ("[Common_SlideShow] Init()");
		...
		if (data != null) {
			...
			if (data.ContainsKey("cards")) {
				...
				//TODO: what's this?
				isFeaturedCardMode = false;

				slideShowName = (string) data ["name"];
	...
	public override void OnPageShown (Dictionary<string, object> data) {
		Debug.Log ("[Common_SlideShow] OnPageShown()");
		DataLoaded();
...
//		NavigationBar.Instance.SetTitle (projectTitle);
		NavigationBar.Instance.SetTitle(slideShowName);
		NavigationBar.Instance.SetRightButton();

	}
	
	private void DataLoaded () {
		Debug.Log ("[Common_SlideShow] DataLoaded()");
//		Debug.Log ("DataLoaded");
//		cards = LocalData.JSONCardsToListCards(cardsNode, isFeaturedCardMode);
		RefreshCurrentCard();
	}
...
*/
	...
	public void EventButtonNextCard () {
		if (_dynCurrentIndex >= cards.Length - 1) return;

		if (_dynPreviousCardController != null) _dynPreviousCardController.SelfDestroy();
...
//		_dynCurrentIndex = LoopIndex(_dynCurrentIndex + 1);
	...
	public void EventButtonPreviousCard () {
		if (_dynCurrentIndex <= 0) return;

		if (_dynNextCardController != null) _dynNextCardController.SelfDestroy();
...
//		_dynCurrentIndex = LoopIndex(_dynCurrentIndex - 1);
	...
	private void RefreshCurrentCard () {
		Debug.Log ("[Common_SlideShow] RefreshCurrentCard()");
		if (cards.Length == 0) {
		...
		//----------------------------------------------------------------------
		// TODO: show loading screen until audio file available?
		//----------------------------------------------------------------------
...
//		_dynCurrentIndex = LoopIndex(_dynCurrentIndex);
		...
		Card currentCard = cards[_dynCurrentIndex];
		CurrentCardController.Init(currentCard, _audioSource, (_dynCurrentIndex+1) , cards.Length, isFeaturedCardMode);
	...
	private CardController CreateEmptyCardController () {
		Debug.Log ("[Common_SlideShow] CreateEmptyCardController()");
...
//	}
	...
	private void RefreshPreviousCard () {
		Debug.Log ("[Common_SlideShow] RefreshPreviousCard()");
//		int idx = LoopIndex(_dynCurrentIndex - 1);
		if (_dynCurrentIndex <= 0) return;
		int idx = _dynCurrentIndex - 1;
		...
		_dynPreviousCardController.Init(currentCard, _audioSource, (idx+1) , cards.Length, isFeaturedCardMode);
	...
	private void RefreshNextCard () {
		Debug.Log ("[Common_SlideShow] RefreshNextCard()");
//		int idx = LoopIndex(_dynCurrentIndex + 1);
		if (_dynCurrentIndex >= cards.Length-1) return;
		int idx = _dynCurrentIndex + 1;
		...
		Card currentCard = cards[idx];
		_dynNextCardController.Init(currentCard, _audioSource, (idx+1) , cards.Length, isFeaturedCardMode);
	...
	private void RenewSpecificProjectCards () {
		Debug.Log ("[Common_SlideShow] RenewSpecificProjectCards()");
		/*
		if (LocalData.Instance.IsTestUser > 0) {
	...
	private void SyncLocalUpdatedCardsToServer () {
		Debug.Log ("[Common_SlideShow] SyncLocalUpdatedCardsToServer()");
		/*
